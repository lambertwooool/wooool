...
from modules.model.ldm.modules.attention import optimized_attention
...
class AttentionSharingPatcher(torch.nn.Module):
    def __init__(self, unet, frames=2, use_control=True, rank=256):
        super().__init__()
        model_loader.unload_model_clones(unet)

        units = []
        for i in range(32):
            real_key = module_mapping_sd15[i]
            attn_module = model_helper.get_attr(unet.model.diffusion_model, real_key)
        ...
        if devices.should_use_fp16(devices.get_torch_device()):
