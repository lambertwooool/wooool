...
from modules.model import ops, clip_model
...
class ClipTokenWeightEncoder:
    def encode_token_weights(self, token_weight_pairs):
        ...
        if pooled is not None:
            first_pooled = pooled[0:1].to(devices.intermediate_device())
        else:
            first_pooled = pooled
        ...
        if (len(output) == 0):
            return out[-1:].to(devices.intermediate_device()), first_pooled
        return torch.cat(output, dim=-2).to(devices.intermediate_device()), first_pooled
