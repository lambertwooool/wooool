...
from modules.model import model_helper, conds
...
class BaseModel(torch.nn.Module):
    ...
    def state_dict_for_saving(self, clip_state_dict, vae_state_dict):
        ...
        unet_state_dict = {}
        for k in unet_sd:
            unet_state_dict[k] = model_helper.resolve_lowvram_weight(unet_sd[k], self.diffusion_model, k)

        unet_state_dict = self.model_config.process_unet_state_dict_for_saving(unet_state_dict)
        vae_state_dict = self.model_config.process_vae_state_dict_for_saving(vae_state_dict)
        if self.get_dtype() == torch.float16:
            clip_state_dict = util.convert_sd_to(clip_state_dict, torch.float16)
            vae_state_dict = util.convert_sd_to(vae_state_dict, torch.float16)
    ...
    def memory_required(self, input_shape):
        ...
        if devices.is_xformers_enable() or devices.is_pytorch_attention_enable():
            ...
            return (area / (devices.dtype_size(self.get_dtype()) * 10)) * (1024 * 1024)
        else:
