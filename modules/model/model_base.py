...
class BaseModel(torch.nn.Module):
    ...
    def extra_conds(self, **kwargs):
        ...
        if len(self.concat_keys) > 0:
            ...
            if concat_latent_image.shape[1:] != noise.shape[1:]:
                concat_latent_image = model_helper.common_upscale(concat_latent_image, noise.shape[-1], noise.shape[-2], "bilinear", "center")
    ...
    def state_dict_for_saving(self, clip_state_dict=None, vae_state_dict=None, clip_vision_state_dict=None):
        ...
        if self.get_dtype() == torch.float16:
            extra_sds = map(lambda sd: model_helper.convert_sd_to(sd, torch.float16), extra_sds)
...
class SVD_img2vid(BaseModel):
    ...
    def extra_conds(self, **kwargs):
        ...
        if latent_image.shape[1:] != noise.shape[1:]:
            latent_image = model_helper.common_upscale(latent_image, noise.shape[-1], noise.shape[-2], "bilinear", "center")

        latent_image = model_helper.resize_to_batch_size(latent_image, noise.shape[0])
