    ...
    def get_control(self, x_noisy, t, cond, batched_number):
        ...
        if self.cond_hint is None or x_noisy.shape[2] * 8 != self.cond_hint.shape[2] or x_noisy.shape[3] * 8 != self.cond_hint.shape[3]:
            if self.cond_hint is not None:
                del self.cond_hint
            self.cond_hint = None
            self.cond_hint = model_helper.common_upscale(self.cond_hint_original, x_noisy.shape[3] * 8, x_noisy.shape[2] * 8, 'nearest-exact', "center").to(self.control_model.dtype).to(self.device)
...
class ControlLora(ControlNet):
    ...
    def pre_run(self, model, percent_to_timestep_function):
        ...
        for k in sd:
            weight = model_helper.resolve_lowvram_weight(sd[k], diffusion_model, k)
            try:
                model_helper.set_attr(self.control_model, k, weight)
            except:
                pass

        for k in self.control_weights:
            if k not in {"lora_controlnet"}:
                model_helper.set_attr(self.control_model, k, self.control_weights[k].to(dtype).to(devices.get_torch_device()))
...
class T2IAdapter(ControlBase):
    ...
    def get_control(self, x_noisy, t, cond, batched_number):
        ...
        if self.cond_hint is None or x_noisy.shape[2] * 8 != self.cond_hint.shape[2] or x_noisy.shape[3] * 8 != self.cond_hint.shape[3]:
            ...
            self.cond_hint = model_helper.common_upscale(self.cond_hint_original, width, height, 'nearest-exact', "center").float().to(self.device)
