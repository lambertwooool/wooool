...
class IPAdapterDetector:
    ...
    def preprocess(self, img, clip_vision):
        ...
        # self.image_emb = cond
        self.image_emb = ip_conds
        self.uncond_image_emb = uncond
    ...
    @torch.no_grad()
    @torch.inference_mode()
    def patch_model(self, model, tasks):
...
class CrossAttentionPatch:
    ...
    def __init__(self, weight, dtype, number, cond, uncond, sdxl):
        self.weights = [weight]
        self.conds = [cond]
        self.unconds = [uncond]
        self.dtype = dtype
    ...
    def set_new_condition(self, weight, cond, uncond, dtype, number, sdxl):
    ...
    def __call__(self, n, context_attn2, value_attn2, extra_options):
        ...
        with torch.autocast("cuda", dtype=self.dtype):
            ...
            for weight, cond, uncond in zip(self.weights, self.conds, self.unconds):
                ip_k_c = cond[self.number * 2].to(q)
                ip_v_c = cond[self.number * 2 + 1].to(q)
                ip_k_uc = ip_unconds[self.number * 2].to(q)
                ip_v_uc = ip_unconds[self.number * 2 + 1].to(q)
